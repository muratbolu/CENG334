...
struct ext2_super_block
{
    ...
    uint32_t
      log_fragment_size;       /* Same for fragments (we won't use fragments) */
    uint32_t blocks_per_group; /* Number of blocks for each block group (last
                                  group can have fewer) */
    uint32_t fragments_per_group; /* Same for fragments */
    uint32_t inodes_per_group;    /* Number of inodes for each block group (last
                                     group can have fewer) */
    uint32_t mount_time; /* Mounting and modification metadata, many less
                            important fields */
    uint32_t write_time;
...
struct ext2_dir_entry
{
    ...
    uint16_t length; /* record length, round up to 4 bytes since records need to
                        be aligned on 4 */
    uint8_t name_length; /* 255 is the maximum possible length */
    uint8_t file_type;   /* Not used in revision 0, file type identifier in
                            revision 1 */
